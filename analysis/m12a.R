...
path <- "/vagrant/results/m13a"
dir.create(path)
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/m13_data.csv",
...
t <- geoML(dta=full.dta,
           ...
           caliper=1.5,
           counterfactual.name = "Stand-alone Bio (MFA + SFA) multi-country",
