...
path <- "/vagrant/results/m8a"
dir.create(path)
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/m8_data.csv",
...
out_path = "/vagrant/results/m8a/"

t <- geoML(dta=full.dta,
           trt=c("treatment", "Programmatic Multi-Agency w/ Bio"),
           ...
           caliper=1.5,
           counterfactual.name = "Non-Programmatic Single-Agency w/ Bio",
