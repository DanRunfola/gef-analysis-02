...
path <- "/vagrant/results/mfa1a"
dir.create(path)
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/mm1_data.csv",
...
out_path = "/vagrant/results/mfa1a/"

t <- geoML(dta=full.dta,
           trt=c("treatment", "Land degradation projects"),
