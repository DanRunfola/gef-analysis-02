...
path <- "/vagrant/results/biom3b"
dir.create(path)
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/bm3_data.csv", 
                     check.names = FALSE, 
...
out_path = "/vagrant/results/biom3b/"

t <- geoML(dta=full.dta,
           trt=c("treatment", "Biodiversity Projects"),
