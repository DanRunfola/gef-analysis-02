...
path <- "/vagrant/results/m11a" #change path ZLV
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/m11_data.csv") #change input data ZLV
...
out_path = "/vagrant/results/m11a/" #change path ZLV
