...
path <- "/vagrant/results/m7b" #change path ZLV
dir.create(path)
...
out_path = "/vagrant/results/m7b/"  #change path ZLV
...
t <- geoML(dta=full.dta,
           ...
           caliper=2.0,
           counterfactual.name = "Non-programmatic single-agency w/ LD", #add control case ZLV
           tree.ctrl = c(2,10),
           col.invert = FALSE,
           tree.cnt = 1000
)
