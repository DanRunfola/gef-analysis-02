...
path <- "/vagrant/results/m9b" #change path ZLV
dir.create(path)
file.remove(file.path(path, list.files(path)))

source("/home/vagrant/geoML/geoML.R")

full.dta <- read.csv("/vagrant/data_prep/analysis_cases/m9_data.csv") #change input data ZLV
...
out_path = "/vagrant/results/m9b/"  #change path ZLV
